...
export default async function handler(request, response) {
  // 프론트엔드에서 보낸 url 파라미터를 가져옵니다. (예: /api/getTitle?url=...)
  ...
  try {
    ...
    // 이 과정은 더 이상 브라우저가 아니므로 CORS나 봇 차단에 훨씬 자유롭습니다.
    ...
    // 받아온 응답에서 HTML 텍스트를 추출합니다.
    const htmlText = await targetResponse.text();

    // cheerio를 이용해 HTML 텍스트를 분석 가능한 객체로 변환합니다.
